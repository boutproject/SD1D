...
NonLocalParallelIntegration::NonLocalParallelIntegration() {
  coord = mesh->coordinates(); // Get the metric tensor 
...
void NonLocalParallelIntegration::initialise(const bool pass_electron_heat_flux_location_is_ylow=false) {
  ...
  *deltal = 0.;
  ...
  Options *options = Options::getRoot()->getSection("non_local_parallel_integration");
...
void NonLocalParallelIntegration::calculateIntegralBelow_cell_centre(const BoutReal &eigenvalue, const Field3D &dimensionless_length_deltas_above, CubicSpline &cubic_spline_inverse_lambdaC, const int &number_of_drive_terms, BoutReal* drive_term_coefficients, CubicSpline* cubic_spline_drive_terms, const int &counter) {
  ...
  do {
	...
	integral_parts[3] = 6*pow(eigenvalue,4) * exp_half_delta_over_eigenvalue_times_two_sinh_half_delta_over_eigenvalue
      ...
      for (int i=0; i<number_of_drive_terms; i++) {
	interp_coeffs_drive_term_centre = cubic_spline_drive_terms[i].coefficients(position);
	...
	integral_coeffs[0] = interp_coeffs_drive_term_centre[0];
	integral_coeffs[1] = interp_coeffs_drive_term_centre[1] / *deltal/interp_coeffs_lambdaC_inverse[0];
	integral_coeffs[2] = interp_coeffs_drive_term_centre[2] / pow(*deltal*interp_coeffs_lambdaC_inverse[0],2)
			      - interp_coeffs_drive_term_centre[1] * interp_coeffs_lambdaC_inverse[1]/2./pow(*deltal,2)/pow(interp_coeffs_lambdaC_inverse[0],3);
	integral_coeffs[3] = interp_coeffs_drive_term_centre[3] / pow(*deltal*interp_coeffs_lambdaC_inverse[0],3)
			      - interp_coeffs_drive_term_centre[2] * interp_coeffs_lambdaC_inverse[1]/pow(*deltal,3)/pow(interp_coeffs_lambdaC_inverse[0],4)
			      + interp_coeffs_drive_term_centre[1] * (0.5*pow(interp_coeffs_lambdaC_inverse[1],2)/pow(*deltal,3)/pow(interp_coeffs_lambdaC_inverse[0],5) - interp_coeffs_lambdaC_inverse[2]/3./pow(*deltal,3)/pow(interp_coeffs_lambdaC_inverse[0],4));
	
	for (int j=0; j<4; j++) integral_below(position->jx,position->jyp,position->jz) += drive_term_coefficients[i]*integral_coeffs[j]*integral_parts[j];
      }      
      position->jy++;
      calc_index(position);
    } while (is_upper_boundary[position->jx] ? position->jy<mesh->yend+1 : position->jy < mesh->yend);
...
void NonLocalParallelIntegration::calculateIntegralAbove_cell_centre(const BoutReal &eigenvalue, const Field3D &dimensionless_length_deltas_above, CubicSpline &cubic_spline_inverse_lambdaC, const int &number_of_drive_terms, BoutReal* drive_term_coefficients, CubicSpline* cubic_spline_drive_terms, const int &counter) {
  ...
  do {
    ...
    calc_index(position);
    if (is_upper_boundary[position->jx])
	...
	//add up the contributions to the integral at jy first from the integral at jy-1 and then from the expansion of the integral between jy-1 and jy
	for (int j=0; j<4; j++) integral_above[*position] += drive_term_coefficients[i]*integral_coeffs[j]*integral_parts[j];
      }
      
      position->jy--;
      calc_index(position);
    } while (is_lower_boundary[position->jx] ? position->jy>mesh->ystart-2 : position->jy>mesh->ystart-1);
    position->jy++;
...
void NonLocalParallelIntegration::calculateIntegralBelow_cell_ylow(const BoutReal &eigenvalue, const Field3D &dimensionless_length_deltas_below, const Field3D &dimensionless_length_deltas_above, CubicSpline &cubic_spline_inverse_lambdaC, const int &number_of_drive_terms, BoutReal* drive_term_coefficients, CubicSpline* cubic_spline_drive_terms_centre, CubicSpline* cubic_spline_drive_terms_ylow, const int &counter) {
  ...
  do {
	...
	interp_coeffs_drive_term_centre = cubic_spline_drive_terms_centre[i].coefficients(position->jx,position->jym,position->jz);
	interp_coeffs_drive_term_ylow = cubic_spline_drive_terms_ylow[i].coefficients(position);
	interp_coeffs_lambdaC_inverse = cubic_spline_inverse_lambdaC.coefficients(position->jx,position->jym,position->jz);
	
	BoutReal integrand_coefficient0 = interp_coeffs_drive_term_centre[0]*(interp_coeffs_drive_term_ylow[0] - 0.5*interp_coeffs_drive_term_ylow[1] + 0.25*interp_coeffs_drive_term_ylow[2] - 0.125*interp_coeffs_drive_term_ylow[3]);
	BoutReal integrand_coefficient1 = interp_coeffs_drive_term_centre[0]*(interp_coeffs_drive_term_ylow[1] - interp_coeffs_drive_term_ylow[2] + 0.75*interp_coeffs_drive_term_ylow[3])
					    + interp_coeffs_drive_term_centre[1]*(interp_coeffs_drive_term_ylow[0] - 0.5*interp_coeffs_drive_term_ylow[1] + 0.25*interp_coeffs_drive_term_ylow[2] - 0.125*interp_coeffs_drive_term_ylow[3]);
	BoutReal integrand_coefficient2 = interp_coeffs_drive_term_centre[0]*(interp_coeffs_drive_term_ylow[2] - 1.5*interp_coeffs_drive_term_ylow[3])
					    + interp_coeffs_drive_term_centre[1]*(interp_coeffs_drive_term_ylow[1] - interp_coeffs_drive_term_ylow[2] + 0.75*interp_coeffs_drive_term_ylow[3])
					    + interp_coeffs_drive_term_centre[2]*(interp_coeffs_drive_term_ylow[0] - 0.5*interp_coeffs_drive_term_ylow[1] + 0.25*interp_coeffs_drive_term_ylow[2] - 0.125*interp_coeffs_drive_term_ylow[3]);
	BoutReal integrand_coefficient3 = interp_coeffs_drive_term_centre[0]*interp_coeffs_drive_term_ylow[3]
					    + interp_coeffs_drive_term_centre[1]*(interp_coeffs_drive_term_ylow[2] - 1.5*interp_coeffs_drive_term_ylow[3])
					    + interp_coeffs_drive_term_centre[2]*(interp_coeffs_drive_term_ylow[1] - interp_coeffs_drive_term_ylow[2] + 0.75*interp_coeffs_drive_term_ylow[3])
					    + interp_coeffs_drive_term_centre[3]*(interp_coeffs_drive_term_ylow[0] - 0.5*interp_coeffs_drive_term_ylow[1] + 0.25*interp_coeffs_drive_term_ylow[2] - 0.125*interp_coeffs_drive_term_ylow[3]);
	BoutReal integrand_coefficient4 = interp_coeffs_drive_term_centre[1]*interp_coeffs_drive_term_ylow[3]
					    + interp_coeffs_drive_term_centre[2]*(interp_coeffs_drive_term_ylow[2] - 1.5*interp_coeffs_drive_term_ylow[3])
					    + interp_coeffs_drive_term_centre[3]*(interp_coeffs_drive_term_ylow[1] - interp_coeffs_drive_term_ylow[2] + 0.75*interp_coeffs_drive_term_ylow[3]);
	BoutReal integrand_coefficient5 = interp_coeffs_drive_term_centre[2]*interp_coeffs_drive_term_ylow[3]
					  + interp_coeffs_drive_term_centre[3]*(interp_coeffs_drive_term_ylow[2] - 1.5*interp_coeffs_drive_term_ylow[3]);
	BoutReal integrand_coefficient6 = interp_coeffs_drive_term_centre[3]*interp_coeffs_drive_term_ylow[3];
	...
	interp_coeffs_drive_term_centre = cubic_spline_drive_terms_centre[i].coefficients(position);
	interp_coeffs_drive_term_ylow = cubic_spline_drive_terms_ylow[i].coefficients(position); // These have already been calculated in the previous loop; it might be slightly better to store them in an array and use them again, but I think this operation is cheap anyway.
	interp_coeffs_lambdaC_inverse = cubic_spline_inverse_lambdaC.coefficients(position);
	...
	BoutReal cubic_integrand_coefficient0 = integrand_coefficient0 - 1./1120.*integrand_coefficient4 - 1./1008.*integrand_coefficient5 - 1./1344.*integrand_coefficient6;
	BoutReal cubic_integrand_coefficient1 = integrand_coefficient1 + 1./28.*integrand_coefficient4 + 25./672.*integrand_coefficient5 + 3./112.*integrand_coefficient6;
	BoutReal cubic_integrand_coefficient2 = integrand_coefficient2 - 9./28.*integrand_coefficient4 - 25./84.*integrand_coefficient5 - 45./224.*integrand_coefficient6;
	BoutReal cubic_integrand_coefficient3 = integrand_coefficient3 + integrand_coefficient4 + 25./36.*integrand_coefficient5 + 5./12.*integrand_coefficient6;
    ...
    // No switch to check for last processor because we calculate mesh->yend+1 value anyway.
...
void NonLocalParallelIntegration::calculateIntegralAbove_cell_ylow(const BoutReal &eigenvalue, const Field3D &dimensionless_length_deltas_below, const Field3D &dimensionless_length_deltas_above, CubicSpline &cubic_spline_inverse_lambdaC, const int &number_of_drive_terms, BoutReal* drive_term_coefficients, CubicSpline* cubic_spline_drive_terms_centre, CubicSpline* cubic_spline_drive_terms_ylow, const int &counter) {
  ...
  do {
	...
	mesh->wait(mesh->irecvYOutOutdest(&integral_above(position->jx,position->jyp,position->jz),1,NONLOCAL_PARALLEL_INTEGRATION_TAGBASE + (mesh->LocalNx - mesh->UpXSplitIndex())*mesh->LocalNz*counter + mesh->LocalNz*(position->jx-mesh->UpXSplitIndex()) + position->jz));
      ...
      // Last grid point for CELL_YLOW quantities is mesh->yend on the last y-processor, so start the calculation from mesh->yend-1 if is_upper_boundary
