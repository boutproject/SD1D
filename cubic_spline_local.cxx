...
void CubicSpline::initialise_periodic(const char &inputdirection) {
  direction = inputdirection;
  if (direction == 'y'){
    
  }
  else throw BoutException("CubicSpline: direction is not valid");
  include_boundary_guard_cells = false;
  upper_boundary_offset = false;
  coeffs = new BoutReal[4];
  for (int i=0; i<4; i++) coeffs[i] = 1./0.;
...
void CubicSpline::calculate(const Field3D &pass_input) {
  // NB if periodic_boundary==false, the closed flux surfaces will use an algorithm identical to the periodic one, as all the other stuff only gets done where there is a BndryLowerY or BndryUpperY
  if (direction=='y') {
    input = pass_input;
    if (periodic_boundary) {
      grad_input = mesh->indexDDY(input,input.getLocation(), DIFF_DEFAULT); // derivative in index space
      mesh->communicate(grad_input); // y-boundary is periodic, so guard cell values are correct automatically
    }
    else if (!include_boundary_guard_cells) {
...
CubicSpline & CubicSpline::operator= (const CubicSpline &rhs) {
  #ifdef CHECK
    if (direction!=rhs.direction || include_boundary_guard_cells!=rhs.include_boundary_guard_cells || upper_boundary_offset!=rhs.upper_boundary_offset || periodic_boundary!=rhs.periodic_boundary)
      throw BoutException("initializations of this CubicSpline and rhs were incompatible: they should not be set equal");
  #endif
  input = rhs.input;
  grad_input = rhs.grad_input;
